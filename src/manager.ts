...
const localStorageItem = localStorage.getItem(
  "sb-rowqaxeeqevtmaoxkqfv-auth-token"
);
...
const selectProductCategory: HTMLSelectElement =
...
const selectCategories: HTMLElement =
...
function addModal() {
  ...
  modalFooter.lastElementChild!.lastElementChild!.textContent =
    "Salvar Produto";
...
function validateProduct() {
  ...
  const categorySelectId =
    selectProductCategory.options[selectProductCategory.selectedIndex];
  ...
  if (!isMaxResumeValid) {
    return showAlert(
      alertModal,
      "O resume não pode ter mais de 100 caracteres",
      "alert-warning"
    );
  }
  ...
  if (!isPriceValid) {
    return showAlert(
      alertModal,
      "O preço deve ser maior que 1 real",
      "alert-warning"
    );
  }
...
function addNewProduct(
  ...
  try {
    ...
    showAlert(
      alertModal,
      "Erro ao adicionar o produto, tenta novamente mais tarde",
      "alert-warning"
    );
  }
}
...
function editModal(btnElement: Element) {
  ...
  const productCategoryId =
    btnElement.parentElement!.parentElement!.getAttribute("id-category")!;
  ...
  try {
    ...
    setTimeout(async () => {
      const res = await fetch(
        `${supabaseURL}/rest/v1/products?id=eq.${idProduct}`,
        {
          method: "PATCH",
          headers: {
            apikey: supabaseKey,
    ...
    showAlert(
      alertModal,
      "Erro ao editar o produto, tenta novamente mais tarde",
      "alert-warning"
    );
  }
}
...
function removeProduct(productId: string) {
  try {
    ...
    showAlert(
      alertModal,
      "Erro ao excluir o produto, tenta novamente mais tarde",
      "alert-warning"
    );
  }
}
